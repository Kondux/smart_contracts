...
describe("Authority", async () => {
    ...
    it("should push a new role", async function () {        
        const newRole = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("newRole"));
        const pushRole = await this.authority.pushRole(this.newGuyAddress, newRole);
        await pushRole.wait();
        expect(await this.authority.roles(this.newGuyAddress)).to.equal(newRole);
    });
