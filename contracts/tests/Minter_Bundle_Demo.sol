...
contract MinterBundleDemo is AccessControl {
    ...
    /// @notice Function to mint a bundle of NFTs if the contract is active. TESTNET ONLY: Payment is not required.
    /// @dev Requires sufficient ETH sent and calls the internal mint function.
    /// @return tokenIds The array of minted token IDs.
    function publicMint() public isActive returns (uint256[] memory){
    ...
    /// @notice Sets the bundle DNA for the minted NFTs. TESTNET Only: This function is not restriceted to admin.
    /// @dev Can only be called by an admin, requires matching array lengths.
    /// @param tokenIds The array of minted token IDs.
    /// @param dnas The array of bundle DNA values.
    function setBundleDna(uint256[] memory tokenIds, uint256[] memory dnas) public {        
