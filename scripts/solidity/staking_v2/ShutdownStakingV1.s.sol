...
contract ShutdownStakingV1Script is Script {
    ...
    function _getNetworkConfig() internal returns (NetworkConfig memory) {
        if (block.chainid == 1) {
            // Ethereum Mainnet
            return NetworkConfig({
                label: "Ethereum Mainnet",
                stakingV1: _resolveEnvAddress("STAKING_V1_ADDRESS", 0x07E6F2239d6FbE2CE00747fCFb8344ebBf973BcC),
                stakingV2: _resolveEnvAddress("STAKING_V2_ADDRESS", address(0)),
                helix: _resolveEnvAddress("HELIX_ADDRESS", 0x69a4A1CD8F2f2c3500F64634B9d69C642e9A5CA4),
                konduxToken: _resolveEnvAddress("KONDUX_TOKEN_ADDRESS", 0x7CA5af5bA3472AF6049F63c1AbC324475D44EFC1),
                authority: _resolveEnvAddress("AUTHORITY_ADDRESS", 0x6A005c11217863c4e300Ce009c5Ddc7e1672150A)
            });
        }
        ...
        if (block.chainid == 11155111) {
            ...
            return NetworkConfig({
                ...
                stakingV1: _resolveEnvAddress("STAKING_V1_ADDRESS", address(0)),
                stakingV2: _resolveEnvAddress("STAKING_V2_ADDRESS", address(0)),
                helix: _resolveEnvAddress("HELIX_ADDRESS", 0xb94F89750d9889656a6D081A0F06cBd3FA3Ad04B),
                konduxToken: _resolveEnvAddress("KONDUX_TOKEN_ADDRESS", 0x2fa9e338CFe579Ff4575BeD2e1Ea407e811F35bc),
                authority: _resolveEnvAddress("AUTHORITY_ADDRESS", 0x685a13093cA561F531c93185B942a3f33385e14E)
            });
        }

        if (block.chainid == 31337) {
            // Local Anvil (forked mainnet) — uses mainnet defaults unless
            // overridden by env vars. Start Anvil with:
            //   anvil --fork-url $MAINNET_RPC_URL
            return NetworkConfig({
                label: "Local Anvil (forked mainnet)",
                stakingV1: _resolveEnvAddress("STAKING_V1_ADDRESS", 0x07E6F2239d6FbE2CE00747fCFb8344ebBf973BcC),
                stakingV2: _resolveEnvAddress("STAKING_V2_ADDRESS", address(0)),
                helix: _resolveEnvAddress("HELIX_ADDRESS", 0x69a4A1CD8F2f2c3500F64634B9d69C642e9A5CA4),
                konduxToken: _resolveEnvAddress("KONDUX_TOKEN_ADDRESS", 0x7CA5af5bA3472AF6049F63c1AbC324475D44EFC1),
        ...
        revert(
            string.concat(
                "ShutdownStakingV1: Unsupported chain ID ",
    ...
    function _validateConfig(NetworkConfig memory config) internal view {
        ...
        // Verify all addresses are deployed contracts (catches misconfigured networks)
        require(config.stakingV1.code.length > 0, "StakingV1 address has no code - not a contract");
        require(config.stakingV2.code.length > 0, "StakingV2 address has no code - not a contract");
        require(config.helix.code.length > 0, "Helix address has no code - not a contract");
        require(config.konduxToken.code.length > 0, "Kondux token address has no code - not a contract");
        require(config.authority.code.length > 0, "Authority address has no code - not a contract");
    }
    ...
    function _shutdownStakingV1(NetworkConfig memory config) internal {
        ...
        // Step 4: Revoke V1's Treasury permissions (prevents claimRewards drain)
        // claimRewards() uses konduxERC20.transferFrom(vault, msg.sender, ...) which
        // only requires ERC20 allowance — Helix roles don't block it. We must revoke
        // V1's RESERVESPENDER permission to prevent reward claims from V1.
        console2.log("  [4/4] Revoking V1 Treasury RESERVESPENDER...");
        address treasuryAddr = address(stakingV1.treasury());
        if (treasuryAddr != address(0) && treasuryAddr.code.length > 0) {
            Treasury treasury = Treasury(payable(treasuryAddr));
            if (treasury.permissions(Treasury.STATUS.RESERVESPENDER, config.stakingV1)) {
                try treasury.setPermission(Treasury.STATUS.RESERVESPENDER, config.stakingV1, false) {
                    console2.log("  [OK] V1 RESERVESPENDER revoked");
                } catch Error(string memory reason) {
                    console2.log("  [WARN] V1 RESERVESPENDER revocation failed:", reason);
                    console2.log("  claimRewards() may still work on V1. Revoke manually if needed.");
                }
            } else {
                console2.log("  [OK] V1 already not RESERVESPENDER");
            }
        } else {
            console2.log("  [WARN] Treasury not found on V1 - cannot revoke RESERVESPENDER");
        }
    ...
    function _enableStakingV2(NetworkConfig memory config) internal {
        ...
        // Step 3: Update Treasury staking contract pointer + ERC20 approval for V2
        // This was deferred from DeployStakingV2 to avoid disrupting V1 while active.
        console2.log("  [3/3] Updating Treasury staking contract to V2...");
        address treasuryAddr = address(Staking(config.stakingV1).treasury());
        if (treasuryAddr != address(0) && treasuryAddr.code.length > 0) {
            Treasury treasury = Treasury(payable(treasuryAddr));
            address currentStaking = treasury.stakingContract();

            if (currentStaking != config.stakingV2) {
                try treasury.setStakingContract(config.stakingV2) {
                    console2.log("  [OK] Treasury stakingContract updated to V2");
                } catch Error(string memory reason) {
                    console2.log("  [WARN] Treasury setStakingContract failed:", reason);
                    console2.log("  V2 reward payouts may not work. Fix manually.");
                }
            } else {
                console2.log("  [OK] Treasury stakingContract already points to V2");
            }

            // Now erc20ApprovalSetup will approve V2 (since stakingContract was just updated)
            try treasury.erc20ApprovalSetup(config.konduxToken, type(uint256).max) {
                console2.log("  [OK] Treasury ERC20 approval granted to V2 (unlimited)");
            } catch Error(string memory reason) {
                console2.log("  [WARN] Treasury erc20ApprovalSetup failed:", reason);
                console2.log("  V2 reward payouts may not work. Fix manually.");
            }
        } else {
            console2.log("  [WARN] Treasury not found - cannot update staking contract");
        }
