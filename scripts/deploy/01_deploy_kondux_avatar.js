...
module.exports = async ({ getNamedAccounts, deployments }) => {
  ...
  const konduxHybridMinterDeployment = await deploy("KonduxHybridMinter", {
    from: deployer,
    args: [
      kNFTAddress,
      foundersPassAddress,
  ...
  console.log(`KonduxHybridMinter deployed to: ${konduxHybridMinterDeployment.address}`);

  // Grant MINTER_ROLE to KonduxHybridMinter on the Kondux NFT contract
  const MINTER_ROLE = ethers.keccak256(ethers.toUtf8Bytes("MINTER_ROLE"));
  console.log(`Granting MINTER_ROLE to KonduxHybridMinter on Kondux NFT contract. Role: ${MINTER_ROLE}`);
  ...
  if (["mainnet", "hardhat"].includes(networkName)) {
    await execute(
      ...
      "grantRole",
      MINTER_ROLE,
      konduxHybridMinterDeployment.address
    );
  ...
  if (["sepolia", "goerli"].includes(networkName)) {
    ...
    await run("verify:verify", {
      address: konduxHybridMinterDeployment.address,
      constructorArguments: [
        kNFTAddress,
        foundersPassAddress,
        treasuryAddress,
...
module.exports.tags = ["KonduxHybridMinter"];
